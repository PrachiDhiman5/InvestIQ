...
const popularTickers: Record<string, { ticker: string; name: string }> = {
 ...
 */
export async function resolveEntity(state: { companyName: string }) {
 ...
 */
export async function fetchFinancials(state: { resolvedEntity: ResolvedEntity }) {
  ...
  if (!financials) {
    const mockData: Record<string, Financials> = {
  ...
  if (financials) {
    ...
    if (financials.revenueGrowthPct !== null) {
      if (financials.revenueGrowthPct > 15) healthScore += 3;
      else if (financials.revenueGrowthPct > 5) healthScore += 2;
      else if (financials.revenueGrowthPct > 0) healthScore += 1;
      else {
        healthScore -= 1;
    ...
    if (financials.debtToEquity !== null) {
      if (financials.debtToEquity < 0.3) healthScore += 3;
      else if (financials.debtToEquity < 1.0) healthScore += 2;
      else if (financials.debtToEquity < 2.0) healthScore += 1;
      else {
        healthScore -= 1;
    ...
    if (financials.profitMargin !== null) {
      if (financials.profitMargin > 20) healthScore += 3;
      else if (financials.profitMargin > 10) healthScore += 2;
      else if (financials.profitMargin > 0) healthScore += 1;
      else {
        healthScore -= 2;
    ...
    if (financials.peRatio !== null) {
      ...
      else if (financials.peRatio > 25) valScore = 4;
      else if (financials.peRatio > 15) valScore = 7;
      else if (financials.peRatio > 0) valScore = 9;
      else {
 ...
 */
export async function fetchNews(state: { resolvedEntity: ResolvedEntity }) {
  ...
  if (tavilyKey && tavilyKey.trim().length > 0) {
    try {
      ...
      if (data && data.results) {
        newsResults = data.results.map((r: any) => ({
...
Do not add any outer wrapper keys. Return ONLY the JSON object.`;
  ...
  const theses = await structuredLlm.invoke(prompt) as { 
    bullCase: CaseAnalysis; 
    bearCase: CaseAnalysis;
    estimatedFinancialHealth: number;
    estimatedValuation: number;
  };

  const rubricScoresUpdate: any = {};
...
Decisiveness Directive:
- Do NOT default to WATCH. Be bold and decisive. Choose INVEST if the growth factors and catalysts outweigh the risks, or PASS if the structural risks or valuation are truly unfavorable. Only use WATCH if there is a specific pending event (e.g. clinical trials, earnings report, acquisition) that will resolve key questions.
...
Do not add any outer wrapper keys. Return ONLY the JSON object.`;
  ...
  const decision = await structuredLlm.invoke(prompt) as JudgeDecision;
