            ...
            for (const [nodeName, nodeOutput] of Object.entries(chunk)) {
                ...
                for (const [key, val] of Object.entries(nodeOutput)) {
                    if (key === "newsResults" || key === "stepLog") {
                        finalState[key] = [...(finalState[key] || []), ...val];
                    }
                ...
                res.write(`data: ${JSON.stringify({
                    ...
                    node: nodeName,
                    output: nodeOutput,
                    currentStepLog: nodeOutput.stepLog || []
                })}\n\n`);
            }
        }
        if (active) {
...
function generateMockReport(companyName, ticker, isPublic) {
    ...
    if (isPublic) {
        if (peVal > 35)
            riskFlags.push("Extreme Valuation Multiple");
        if (debtVal > 1.0)
            riskFlags.push("Elevated Debt leverage");
        if (growthVal < 6.0)
            riskFlags.push("Sluggish Growth Vector");
    }
    else {
...
async function streamReplay(res, state, active) {
    // 1. resolveEntity
    if (!active)
        return;
    res.write(`data: ${JSON.stringify({
        ...
        currentStepLog: state.stepLog.filter((l) => l.includes("Resolved") || l.includes("resolver"))
    })}\n\n`);
    await new Promise(r => setTimeout(r, 200));
    // 2. fetchFinancials
    if (!active)
        return;
    const financialLogs = state.stepLog.filter((l) => l.includes("Fetched financials") || l.includes("financial rubric") || l.includes("financial databases"));
    res.write(`data: ${JSON.stringify({
        event: "step",
    ...
    res.write(`data: ${JSON.stringify({
        ...
        currentStepLog: state.stepLog.filter((l) => l.includes("news articles") || l.includes("sentiment rubric") || l.includes("Sentiment"))
    })}\n\n`);
    await new Promise(r => setTimeout(r, 200));
    // 4. buildBullCase
    if (!active)
    ...
    res.write(`data: ${JSON.stringify({
        ...
        currentStepLog: state.stepLog.filter((l) => l.includes("bullish") || l.includes("catalysts"))
    })}\n\n`);
    await new Promise(r => setTimeout(r, 200));
    // 5. buildBearCase
    if (!active)
    ...
    res.write(`data: ${JSON.stringify({
        ...
        currentStepLog: state.stepLog.filter((l) => l.includes("bearish") || l.includes("risk factors") || l.includes("structural risks"))
    })}\n\n`);
    await new Promise(r => setTimeout(r, 200));
    // 6. judge
    if (!active)
    ...
    res.write(`data: ${JSON.stringify({
        ...
        currentStepLog: state.stepLog.filter((l) => l.includes("Judge") || l.includes("deliberation") || l.includes("verdict") || l.includes("Verdict"))
    })}\n\n`);
    await new Promise(r => setTimeout(r, 200));
}
