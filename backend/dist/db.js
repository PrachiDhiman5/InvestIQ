...
// Get cached analysis
export async function getCachedAnalysis(companyName) {
    const normalized = companyName.trim().toLowerCase();
    if (useLocalCache || !pgPool) {
        return getLocalCache(normalized);
    }
    try {
        const res = await pgPool.query("SELECT * FROM investment_research_cache WHERE LOWER(company_name) = $1", [normalized]);
...
export async function saveAnalysisToCache(record) {
    ...
    try {
        await pgPool.query(`INSERT INTO investment_research_cache 
...
// Local cache helpers
function getLocalCache(companyName) {
